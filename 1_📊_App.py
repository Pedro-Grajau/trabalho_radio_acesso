...
with st.sidebar:
    st.subheader("Entre com as informações :page_facing_up:")
    ...
    Hm = st.number_input(label = "Entre com a altura do receptor (m)", value=1.5)
...
if st.button('Calcular'):
    if d > dp:
        ...
        st.error("A distância deve estar entre 1 e 20KM")
    elif f > 2200 or f < 150:
        st.error("A frequencia permitida está entre 150MHz e 2200MHz")
    elif Hb < 30 or Hb > 200:
        st.error("A altura da ERB permitida está entre 30 e 200 metros")
    elif Hm < 1 or Hm > 10: 
        st.error("A altura do receptor permitida está entre 1 e 10 metros")
    else:
